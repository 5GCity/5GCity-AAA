...
class KeyCloakBuilder(Command):
    ...
    def __build_docker_command__(mcs, realm_name):
        ...
        command = (
            ...
            f"-e DB_DATABASE={ConfReader().get('keycloack', 'DB_DATABASE')} "
            f"-e DB_PASSWORD={ConfReader().get('keycloack', 'DB_PASSWORD')} "
            f"-e DB_USER={ConfReader().get('keycloack', 'DB_USER')} "
            f"-e DB_VENDOR={ConfReader().get('keycloack', 'DB_VENDOR')} "
            f"-e DB_ADDR={postgres.attrs['NetworkSettings']['Networks']['aaa_compose_default']['IPAddress']} "
            f"-e DB_PORT={ConfReader().get('keycloack', 'DB_PORT')} "
            "jboss/keycloak "
